...
public abstract class DateHelper {
    ...
    public static List<Date> getMonthsBetween(final Date startDate, final Date endDate) {
        ...
        for (int i = 0; i < years.size(); i++) {
            for (int month = i == 0 ? get(startDate, Calendar.MONTH)
                    : 0; month <= (i == years.size() - 1 ? get(endDate, Calendar.MONTH) : 11); month++) {
                final Calendar calendar = Calendar.getInstance();
    ...
    public static List<Date> getWeeksBetween(final Date startDate, final Date endDate) {
        ...
        for (int i = 0; i < months.size(); i++) {
            for (int week = i == 0 ? get(startDate, Calendar.WEEK_OF_MONTH) : 1; week <= (i == months.size() - 1
                    ? get(endDate, Calendar.WEEK_OF_MONTH)
                    : toCalendar(months.get(i)).getActualMaximum(Calendar.WEEK_OF_MONTH)); week++) {
                final Calendar calendar = Calendar.getInstance();
    ...
    public static List<Date> getDaysBetween(final Date startDate, final Date endDate) {
        ...
        for (int i = 0; i < months.size(); i++) {
            for (int day = i == 0 ? get(startDate, Calendar.DAY_OF_MONTH) : 1; day <= (i == months.size() - 1
                    ? get(endDate, Calendar.DAY_OF_MONTH)
                    : toCalendar(months.get(i)).getActualMaximum(Calendar.DAY_OF_MONTH)); day++) {
                final Calendar calendar = Calendar.getInstance();
