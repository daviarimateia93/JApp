...
public class Proxy {
    ...
    protected static MethodInterceptor newMethodInterceptor() {
        return new MethodInterceptor() {
            @Override
            public Object intercept(final Object instance, final Method method, final Object[] parameters,
                    final MethodProxy methodProxy) throws Throwable {
                final Class<?> type = method.getDeclaringClass();

                if (ProxyInterceptable.class.isAssignableFrom(type) && isInterceptable(method)) {
                    final Method interceptMethod = ReflectionHelper.getMethod(type, "intercept",
                            new Class<?>[] { ProxyMethodWrapper.class });

                    return interceptMethod.invoke(instance,
                            new ProxyMethodWrapper(methodProxy, instance, method, parameters));
                } else {
