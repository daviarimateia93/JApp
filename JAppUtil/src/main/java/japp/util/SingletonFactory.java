...
public class SingletonFactory {
    ...
    public static <T extends Singletonable & ProxyInterceptable> Reference<T> getProxyInterceptableInstance(
            final Class<T> instanceClass, final Class<?>[] parameterClasses, final Object... parameters) {
        return getInstance(instanceClass, new Callable<T>() {
            @Override
            public T call() throws Exception {
    ...
    public static <T extends Singletonable & ProxyInterceptable> Reference<T> getProxyInterceptableInstancePerThread(
            final Class<T> instanceClass, final Class<?>[] parameterClasses, final Object... parameters) {
        return getInstancePerThread(instanceClass, new Callable<T>() {
            @Override
            public T call() throws Exception {
    ...
    protected static <T extends Singletonable> Reference<T> getInstance(final Class<T> instanceClass,
            final Callable<T> callable) {
        return getInstance(instances, instanceClass, callable);
    }

    protected static <T extends Singletonable> Reference<T> getInstancePerThread(final Class<T> instanceClass,
            final Callable<T> callable) {
    ...
    protected static <T extends Singletonable> Reference<T> getInstance(
            final Map<Class<? extends Singletonable>, Reference<? extends Singletonable>> map,
            final Class<T> instanceClass, final Callable<T> callable) {
        try {
